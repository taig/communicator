...
/**
 * A {@link Map Map&lt;String, Object[]&gt;} that acts as container for request- and response headers with additional
 * serialization methods.
 * <p/>
 * This class also provides a variety of fields to represent all HTTP-headers that may occur in a request- and a
 * response-header.
 *
 * @see #mkString(String)
 */
public class Header extends HashMap<String, Object[]> implements Appliable
{
	...
	/**
	 * Construct a {@link Header} object with one predefined key/value pair.
	 *
	...
	/**
	 * Add a key/value pair to the {@link Header}.
	 *
	 * @param key    The key to add.
	 * @param values The values to add.
	 * @return The values that have previously been associated with the given key.
	 */
	...
	/**
	 * Append the given values to the values that are currently associated with the key. If the key does not exist yet
	 * this method performs a simple {@link #put(String, Object...)} operation.
	 *
	...
	/**
	 * Create a String representation of the {@link Header} with an arbitrary value delimiter.
	 *
	 * @param delimiter The delimiter used to separate the Header's values.
	 * @return The HEader's String representation.
	 */
	...
	/**
	 * Create a key/value String representation of a key and an arbitrary array.
	 *
	 * @param key       The key.
	 * @param values    The values that will be concatenated.
	 * @param delimiter The delimiter used to separate the array's entries.
	 * @return The key followed by a colon and a space character as well as the array's String values concatenated with
	 * the supplied delimiter.
	 * @see #mkString(Object[], String)
	 */
	...
	/**
	 * Concat an arbitrary array with a given delimiter.
	 *
	 * @param values    The array that will be transformed to a String representation.
	 * @param delimiter The delimiter used to separate the array's entries.
	 * @return The array's String values concatenated with the supplied delimiter.
	 */
	...
	public static abstract class Request extends Header
	{
		...
		/**
		 * The domain name of the server (for virtual hosting), and the TCP port number on which the server is
		 * listening. The
		 * port number may be omitted if the port is the standard port for the service requested. Mandatory since
		 * HTTP/1.1.
		 */
		 ...
		 * Transfer-Encoding can be used, plus the "trailers" value (related to the "<a
		 * href="http://en.wikipedia.org/wiki/Chunked_transfer_encoding
		 * ">chunked</a>" transfer method) to notify the server it expects to receive additional headers (the trailers)
		 * after
		...
		/**
		 * Allows easier parsing of the MakeModel/Firmware that is usually found in the User-Agent String of AT&T
		 * Devices.
		 */
		...
		/**
		 * Mainly used to identify Ajax requests. Most JavaScript frameworks send this header with value of
		 * XMLHttpRequest.
		 */
		public static final String X_REQUESTED_WITH = "X-Requested-With";

		/**
		 * Links to an XML file on the Internet with a full description and details about the device currently
		 * connecting. In
		...
		/**
		 * A utility class that helps to generate a proper value for the HTTP field <code>Content-Type</code>.
		 */
		public static class ContentType
		{
			...
			/**
			 * Construct a {@link ContentType} object.
			 *
			 * @param type The HTTP-fields base value such as <code>application/x-www-form-urlencoded</code> or
			...
			/**
			 * Retrieve the {@link ContentType ContentType's} type value.
			 * 
			 * @return The ContentType's type value.
			 */
	...
	public static abstract class Response extends Header
	{
		...
		/**
		 * The last modified date for the requested object, in <a href="http://tools.ietf.org/html/rfc2822">RFC
		 * 2822</a>
		 * format.
		 */
		 ...
		 * P3P:CP="your_compact_policy". However, P3P did not take off, most browsers have never fully implemented it,
		 * a
		 * lot
		 * of websites set this header with fake policy text, that was enough to fool browsers the existence of P3P
		 * policy and
		...
		/**
		 * Used in redirection, or when a new resource has been created. This refresh redirects after 5 seconds. This
		 * is
		 * a
		...
		/**
		 * If an entity is temporarily unavailable, this instructs the client to try again after a specified period of
		 * time
		 * (seconds).
		 */
		 ...
		 * message encoded with <a href="http://en.wikipedia.org/wiki/Chunked_transfer_encoding">chunked
		 * transfer-coding</a>.
		 */
		...
		/**
		 * Tells downstream proxies how to match future request headers to decide whether the cached response can be
		 * used
		...
		/**
		 * <a href="http://en.wikipedia.org/wiki/Clickjacking">Clickjacking</a> protection: "deny" - no rendering
		 * within
		 * a
		...
		/**
		 * Specifies the technology (e.g. ASP.NET, PHP, JBoss) supporting the web application (version details are
		 * often
		 * in
		...
		/**
		 * Recommends the preferred rendering engine (often a backward-compatibility mode) to use to display the
		 * content. Also
